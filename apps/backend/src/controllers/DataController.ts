...
const accountSchema = z.object({
  ...
  type: z.enum(['checking', 'savings', 'credit', 'cash', 'investment'], { required_error: 'Invalid account type' }),
...
const budgetSchema = z.object({
  category_id: z.string().uuid('Invalid category ID'),
  amount: z.number().positive('Amount must be positive'),
  period: z.enum(['weekly', 'monthly', 'yearly'], { required_error: 'Invalid period' }),
...
export class DataController {
  ...
  createCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const categoryId = await this.googleSheetsService.insert(spreadsheetId, 'categories', categoryData);
  ...
  getCategories = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const categories = await this.googleSheetsService.find(spreadsheetId, 'categories', {
        user_id: user.email
      });
  ...
  updateCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await this.googleSheetsService.update(spreadsheetId, 'categories', id, validatedData);
  ...
  createAccount = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const accountId = await this.googleSheetsService.insert(spreadsheetId, 'accounts', accountData);
  ...
  getAccounts = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const accounts = await this.googleSheetsService.find(spreadsheetId, 'accounts', {
        user_id: user.email
      });
  ...
  updateAccount = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await this.googleSheetsService.update(spreadsheetId, 'accounts', id, validatedData);
  ...
  createTransaction = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transactionId = await this.googleSheetsService.insert(spreadsheetId, 'transactions', transactionData);

      // Update account balance
      const account = await this.googleSheetsService.findById(spreadsheetId, 'accounts', validatedData.account_id);
      if (account) {
        const newBalance = validatedData.type === 'income' 
          ? parseFloat(account.balance) + validatedData.amount
          : parseFloat(account.balance) - validatedData.amount;
        
        await this.googleSheetsService.update(spreadsheetId, 'accounts', validatedData.account_id, {
          balance: newBalance
        });
      }
  ...
  getTransactions = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let transactions = await this.googleSheetsService.find(spreadsheetId, 'transactions', filters);
  ...
  updateTransaction = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await this.googleSheetsService.update(spreadsheetId, 'transactions', id, validatedData);
  ...
  deleteTransaction = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const transaction = await this.googleSheetsService.findById(spreadsheetId, 'transactions', id);
      
      await this.googleSheetsService.delete(spreadsheetId, 'transactions', id);

      // Adjust account balance
      if (transaction) {
        const account = await this.googleSheetsService.findById(spreadsheetId, 'accounts', transaction.account_id);
        if (account) {
          ...
          await this.googleSheetsService.update(spreadsheetId, 'accounts', transaction.account_id, {
            balance: newBalance
          });
        }
      }
  ...
  getDashboard = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [accounts, transactions, categories, budgets] = await Promise.all([
        this.googleSheetsService.find(spreadsheetId, 'accounts', { user_id: user.email }),
        this.googleSheetsService.find(spreadsheetId, 'transactions', { user_id: user.email }),
        this.googleSheetsService.find(spreadsheetId, 'categories', { user_id: user.email }),
        this.googleSheetsService.find(spreadsheetId, 'budgets', { user_id: user.email })
      ]);
